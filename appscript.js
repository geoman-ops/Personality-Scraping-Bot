...
function filterResults(arrResults, company_1, company_2, firstName, lastName) {
    ...
    /* Compare Strong and Super Match Results*/
...
function getLinkedinUrl(row) {
    ...
    let linkedinUrlempty = SpreadsheetApp.getActiveSheet().getRange("G" + row).isBlank();
    ...
    if (linkedinUrlempty) {
        ...
        Logger.log("There is already a linkedIn Url on line: " + row);
        return true;
    }
...
function Process_All_Rows_Get_LinkedIn_Url() {
    ...
    let lastSpreadsheetRow = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Variables').getRange("A2").getValues(); //Get the last row that has a SerpWow search URL 

    for (row = lastIndexedRow; row <= lastSpreadsheetRow; row++) {
        SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Variables').getRange("B2").setValue(row);
        if (i <= 1) {  //Change this number to change the max number of created profiles
...
// /* Process all rows and Get linkedIn Url - Replace strong Match Results*/

//   function Process_All_Rows_Get_LinkedIn_Url() {
//     Logger.log("I started!");
//       var i = 0;

//       let lastIndexedRow = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Variables').getRange("B2").getValues(); // Get the value of last row that has been indexed (spreadsheet name: variables, column/cell B2)
//       // var intLastIttRow = parseInt(lastIndexedRow, 10);
//       Logger.log("lastindexedrow ="+lastIndexedRow);
//       let lastSpreadsheetRow = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Variables').getRange("A2").getValues(); //Get the last row that has a SerpWow search URL
//       Logger.log("lastspreadsheetRow ="+lastSpreadsheetRow);

//       for (let row = lastIndexedRow; row <= 2638; row++) {
//         Logger.log("Started the itteration! Hooray!!");
//           SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Variables').getRange("B2").setValue(row);
//           let isStrongMatch = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Sheet1').getRange("H"+row).getValues();
//           if (isStrongMatch == 90 || isStrongMatch == 60 || isStrongMatch == 30 || isStrongMatch == 15) {
//           if (i <= 3) {  //Change this number to change the max number of created profiles
//             SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Sheet1').getRange("G"+row).clearContent();
//               if (getLinkedinUrl(row)) {
...
//           }
//           } else {
//         Logger.log("There is not a strong match on row "+row+" . We move forward to the next row");
//       }
//       }

//       Logger.log("Did not starhed the iterration! :-( ");
//   }
